...
public class Main {
    ...
    static final boolean newTextBasedOldOne = false;
    ...
    private static boolean handleNewText(Scanner scanner){
        ...
        text = scanner.nextLine();
        if(newTextBasedOldOne && (oldTextLength != 0 && !IsSameCharacterSet())){
            System.out.println("The Valid Input");
            text = "";
            return true;
        }
        ASCII = new int[128];
    ...
    private static void decodedText(){
        decoded = "";
    ...
    private static void encodedText(){
        encoded = "";
        for (int i = 0; i < text.length();i++){
    ...
    private static void calculateCharIntervals(PriorityQueue<Node> vector, boolean printIntervals){
        if(printIntervals) System.out.println("----Interval----");
